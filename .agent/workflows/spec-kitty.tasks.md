...
## вљ пёЏ CRITICAL: THIS IS THE MOST IMPORTANT PLANNING WORK
...
## рџ"Ќ WORKING DIRECTORY: Stay in planning repository
...
# Creates:
# - kitty-specs/###-feature/tasks/WP01-*.md в†' In planning repository
# - kitty-specs/###-feature/tasks/WP02-*.md в†' In planning repository
...
**Expected output:** the target branch (meta.json в†' target_branch), typically `main` or `2.x`
...
1. **Setup**: Run `spec-kitty agent feature check-prerequisites --json --paths-only --include-tasks` from the repository root and capture `FEATURE_DIR` plus `AVAILABLE_DOCS`. All paths must be absolute.
   ...
   **DO NOT CREATE** paths like:
   - вќЊ `tasks/WP01-slug.md` (missing FEATURE_DIR prefix)
   - вќЊ `/tasks/WP01-slug.md` (wrong root)
   - вќЊ `FEATURE_DIR/tasks/planned/WP01-slug.md` (WRONG - no subdirectories!)
   - вќЊ `WP01-slug.md` (wrong directory)
...
4. **Roll subtasks into work packages** (IDs `WP01`, `WP02`, ...):
   ...
   **NUMBER OF WPs**: Let the work dictate the count
   ...
   - Complex feature (50+ subtasks): 10-20 WPs в†ђ **This is OK!**
   ...
   **GROUPING PRINCIPLES**:
   ...
   - Name with succinct goal (e.g., "User Story 1 вЂ" Real-time chat happy path")
...
- вњ… **20 WPs of 5 subtasks each** = 100 subtasks, manageable prompts
- вќЊ **5 WPs of 20 subtasks each** = 100 subtasks, overwhelming 1400-line prompts

**Feature complexity scales with subtask count, not WP count**:
- Simple feature: 10-15 subtasks в†' 2-4 WPs
- Medium feature: 30-50 subtasks в†' 6-10 WPs
- Complex feature: 80-120 subtasks в†' 15-20 WPs в†ђ **Totally fine!**
- Very complex: 150+ subtasks в†' 25-30 WPs в†ђ **Also fine!**
...
3. **Prioritisation & dependencies**:
   - Sequence work packages: setup в†' foundational в†' story phases (priority order) в†' polish.
...
4. **Prompt composition**:
   ...
   - Provide actionable implementation and test guidance per subtaskвЂ"short for trivial work, exhaustive for complex flows.
...
For each cohesive unit of work:
  ...
  2. Count subtasks
  3. Estimate prompt lines (subtasks Г--- 50 lines avg)

  If subtasks <= 7 AND estimated lines <= 500:
    вњ" Good WP size - create it

  Else if subtasks > 10 OR estimated lines > 700:
    вњ--- Too large - split into 2+ WPs

  Else if subtasks < 3 AND can merge with related WP:
    в†' Consider merging (but don't force it)
```

**Examples**:

**Good sizing**:
- WP01: Database Foundation (5 subtasks, ~300 lines) вњ"
- WP02: User Authentication (7 subtasks, ~450 lines) вњ"
- WP03: Admin Dashboard (6 subtasks, ~400 lines) вњ"

**Too large - MUST SPLIT**:
- вќЊ WP01: Entire Backend (25 subtasks, ~1500 lines)
  - вњ" Split into: DB Layer (5), Business Logic (6), API Layer (7), Auth (7)

**Too small - CONSIDER MERGING**:
- WP01: Add config file (2 subtasks, ~100 lines)
- WP02: Add logging (2 subtasks, ~120 lines)
  - вњ" Merge into: WP01: Infrastructure Setup (4 subtasks, ~220 lines)
...
**Self-check**:
- Subtask count: 3-7? вњ" | 8-10? вљ пёЏ | 11+? вќЊ SPLIT
- Estimated lines: 200-500? вњ" | 500-700? вљ пёЏ | 700+? вќЊ SPLIT
- Can implement in one session? вњ" | Multiple sessions needed? вќЊ SPLIT
...
- **Size validation** (e.g., "вњ" All WPs within ideal range" OR "вљ пёЏ WP05 is 820 lines - consider splitting")
...
### вќЊ MISTAKE 1: Optimizing for WP Count

**Bad thinking**: "I'll create exactly 5-7 WPs to keep it manageable"
в†' Results in: 20 subtasks per WP, 1200-line prompts, overwhelmed agents

**Good thinking**: "Each WP should be 3-7 subtasks (200-500 lines). If that means 15 WPs, that's fine."
в†' Results in: Focused WPs, successful implementation, happy agents

### вќЊ MISTAKE 2: Token Conservation During Planning

**Bad thinking**: "I'll save tokens by writing brief prompts with minimal guidance"
в†' Results in: Agents confused during implementation, asking clarifying questions, doing work wrong, requiring rework

**Good thinking**: "I'll invest tokens now to write thorough prompts with examples and edge cases"
в†' Results in: Agents implement correctly the first time, no rework needed, net token savings

### вќЊ MISTAKE 3: Mixing Unrelated Concerns
...
### вќЊ MISTAKE 4: Insufficient Prompt Detail
