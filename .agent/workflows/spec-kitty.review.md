...
```bash
spec-kitty agent workflow review {{args}} --agent <your-name>
```
...
- вњ… Approve: `spec-kitty agent tasks move-task WP## --to done --note "Review passed: <summary>"`
- вќЊ Reject: Write feedback to the temp file path shown in the prompt, then run `spec-kitty agent tasks move-task WP## --to planned --review-feedback-file <temp-file-path>`
