...
# Creates:
# - kitty-specs/###-feature/plan.md в†' In planning repository
...
- Verify you're on the target branch (meta.json в†' target_branch) before scaffolding plan.md
...
2. **Detect feature context** (CRITICAL - prevents wrong feature selection):
   ...
   a. **Check git branch name**:
      ...
      - Example: Branch `020-my-feature` or `020-my-feature-WP01` в†' Feature `020-my-feature`

   b. **Check current directory**:
      - Look for `###-feature-name` pattern in the current path
      - Examples:
        - Inside `kitty-specs/020-my-feature/` в†' Feature `020-my-feature`
        - Not in a worktree during planning (worktrees only used during implement): If detection runs from `.worktrees/020-my-feature-WP01/` в†' Feature `020-my-feature`
...
5. **Execute plan workflow**: Follow the structure in IMPL_PLAN template, using the validated planning answers as ground truth:
   - Update Technical Context with explicit statements from the user or discovery research; mark `[NEEDS CLARIFICATION: вЂ¦]` only when the user deliberately postpones a decision
...
6. **STOP and report**: This command ends after Phase 1 planning. Report branch, IMPL_PLAN path, and generated artifacts.
   ...
   **вљ пёЏ CRITICAL: DO NOT proceed to task generation!** The user must explicitly run `/spec-kitty.tasks` to generate work packages. Your job is COMPLETE after reporting the planning artifacts.
...
1. **Extract unknowns from Technical Context** above:
   - For each NEEDS CLARIFICATION в†' research task
   - For each dependency в†' best practices task
   - For each integration в†' patterns task
...
2. **Generate API contracts** from functional requirements:
   - For each user action в†' endpoint
...
Do NOT:
- вќЊ Generate `tasks.md`
- вќЊ Create work package (WP) files
- вќЊ Create `tasks/` subdirectories
- вќЊ Proceed to implementation
