...
## в›" Location Pre-flight Check (CRITICAL)
...
в›" **STOP - DANGER! You are in the wrong location!**
...
if not result.is_valid:
    ...
    print('вњ" Location verified:', result.branch_name)
"
```
...
1. вњ… All work packages must be in `done` lane (reviewed and approved)
2. вњ… Feature must pass `/spec-kitty.accept` checks
3. вњ… Working directory must be clean (no uncommitted changes in main)
4. вњ… **You must be in main repository root** (not in a worktree)
...
```
вњ… Preserves full commit history
вњ… Clear feature boundaries in git log
вќЊ More commits in main branch
...
spec-kitty merge --strategy squash
```
вњ… Clean, linear history on main
вњ… Single commit per feature
вќЊ Loses individual commit details
...
```
вњ… Linear history without merge commits
вќЊ Requires manual intervention
вќЊ Rewrites commit history
...
в"њв"Ђв"Ђ .worktrees/
в"‚   в"њв"Ђв"Ђ 001-auth-system-WP01/           # WP01 worktree
в"‚   в"њв"Ђв"Ђ 001-auth-system-WP02/           # WP02 worktree
в"‚   в"њв"Ђв"Ђ 001-auth-system-WP03/           # WP03 worktree
в"‚   в""в"Ђв"Ђ 002-dashboard-WP01/             # Different feature
в"њв"Ђв"Ђ .kittify/
в"њв"Ђв"Ђ kitty-specs/
в""в"Ђв"Ђ ... (main branch files)
```
...
my-project/                    # Main repo (main branch)
в"њв"Ђв"Ђ .worktrees/
в"‚   в"њв"Ђв"Ђ 001-auth-system/      # Feature 1 worktree (single)
в"‚   в"њв"Ђв"Ђ 002-dashboard/        # Feature 2 worktree (single)
в"‚   в""в"Ђв"Ђ 003-notifications/    # Feature 3 worktree (single)
в"њв"Ђв"Ђ .kittify/
в"њв"Ђв"Ђ kitty-specs/
в""в"Ђв"Ђ ... (main branch files)
```
...
### Why Worktrees?
- вњ… Work on multiple features simultaneously
- вњ… Each feature has its own sandbox
- вњ… No branch switching in main repo
- вњ… Easy to compare features
- вњ… Clean separation of concerns

### The Flow
```
1. /spec-kitty.specify           в†' Creates branch + worktree
2. cd .worktrees/<feature>/      в†' Enter worktree
3. /spec-kitty.plan              в†' Work in isolation
4. /spec-kitty.tasks
5. /spec-kitty.implement
6. /spec-kitty.review
7. /spec-kitty.accept
8. /spec-kitty.merge             в†' Merge + cleanup worktree
9. Back in main repo!            в†' Ready for next feature
```
...
- вњ… Feature code integrated
- вњ… Worktree removed (if it existed)
- вњ… Feature branch deleted (unless `--keep-branch`)
- вњ… Ready to start your next feature!
...
```
specify в†' plan в†' tasks в†' implement в†' review в†' accept в†' merge вњ…
```