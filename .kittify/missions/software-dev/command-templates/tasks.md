...
## [WARNING]️ CRITICAL: THIS IS THE MOST IMPORTANT PLANNING WORK
...
1. **Setup**: Run `spec-kitty agent feature check-prerequisites --json --paths-only --include-tasks` from the repository root and capture `FEATURE_DIR` plus `AVAILABLE_DOCS`. All paths must be absolute.
   ...
   **DO NOT CREATE** paths like:
   - [NO] `tasks/WP01-slug.md` (missing FEATURE_DIR prefix)
   - [NO] `/tasks/WP01-slug.md` (wrong root)
   - [NO] `FEATURE_DIR/tasks/planned/WP01-slug.md` (WRONG - no subdirectories!)
   - [NO] `WP01-slug.md` (wrong directory)
...
- [NO] **5 WPs of 20 subtasks each** = 100 subtasks, overwhelming 1400-line prompts
...
- [NO] WP01: Entire Backend (25 subtasks, ~1500 lines)
...
**Self-check**:
- Subtask count: 3-7? ✓ | 8-10? [WARNING]️ | 11+? [NO] SPLIT
- Estimated lines: 200-500? ✓ | 500-700? [WARNING]️ | 700+? [NO] SPLIT
- Can implement in one session? ✓ | Multiple sessions needed? [NO] SPLIT
...
- **Size validation** (e.g., "✓ All WPs within ideal range" OR "[WARNING]️ WP05 is 820 lines - consider splitting")
...
## [WARNING]️ Common Mistakes to Avoid

### [NO] MISTAKE 1: Optimizing for WP Count
...
### [NO] MISTAKE 2: Token Conservation During Planning
...
### [NO] MISTAKE 3: Mixing Unrelated Concerns
...
### [NO] MISTAKE 4: Insufficient Prompt Detail
